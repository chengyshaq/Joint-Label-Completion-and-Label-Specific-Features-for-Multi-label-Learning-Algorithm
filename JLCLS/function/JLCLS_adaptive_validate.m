...
function [ BestParameter, BestResult ] = JLCLS_adaptive_validate( train_data, train_target, oldOptmParameter)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% tune the best parameters for JLCLS by crossvalidation
% Input
...
%     theta_searchrange     = oldOptmParameter.theta_searchrange;
    ...
    for i=1:length(alpha_searchrange) % alpha
        for j=1:length(beta_searchrange) % beta
            for k = 1:length(gamma_searchrange) % gamma
                    fprintf('\n-   %d-th/%d: search parameter alpha and beta for JLCLS, alpha = %f, beta = %f, and gamma = %f',index, total, alpha_searchrange(i), beta_searchrange(j), gamma_searchrange(k));
                    index = index + 1;
                    ...
                    optmParameter.epsilon = 10^-6;
                    optmParameter.rho     = 10^-5;
                    optmParameter.p       = 1.1;
                    Result = zeros(15,1);
                    for cv = 1:num_cv
